...
async function identify(shard) {
    ...
    if (shard.state === types_js_1.ShardState.Connected) {
        console.log("CLOSING EXISTING SHARD: #" + shard.id);
        shard.close(types_js_1.ShardSocketCloseCodes.ReIdentifying, "Re-identifying closure of old connection.");
    }
    ...
    shard.send({
        op: shared_js_1.GatewayOpcodes.Identify,
        d: {

"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Embeds = exports.embedLimits = void 0;
require("../../_dnt.polyfills.js");
const routes_js_1 = require("../../util/routes.js");
const deps_js_1 = require("./deps.js");
exports.embedLimits = {
    title: 256,
    description: 4096,
    fieldName: 256,
    fieldValue: 1024,
    footerText: 2048,
    authorName: 256,
    fields: 25,
    total: 6000,
};
// COPIED FROM DISCORDENO AS A TEMP SOLUTION
// TODO: remove when DD has improved utils
function avatarURL(userId, discriminator, options) {
    return options?.avatar
        ? (0, deps_js_1.formatImageURL)(routes_js_1.routes.USER_AVATAR(userId, typeof options?.avatar === "string" ? options.avatar : (0, deps_js_1.iconBigintToHash)(options?.avatar)), options?.size || 128, options?.format)
        : routes_js_1.routes.USER_DEFAULT_AVATAR(Number(discriminator) % 5);
}
class Embeds extends Array {
    constructor(enforceLimits = true) {
        super();
        /** The amount of characters in the embed. */
        Object.defineProperty(this, "currentTotal", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        /** Whether the limits should be enforced or not. */
        Object.defineProperty(this, "enforceLimits", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: true
        });
        /** If a file is attached to the message it will be added here. */
        Object.defineProperty(this, "file", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        // By default, always enforce discord limits but this allows bypass for any reason.
        if (!enforceLimits)
            this.enforceLimits = false;
        return this;
    }
    fitData(data, max) {
        // If the string is bigger then the allowed max shorten it.
        if (data.length > max)
            data = data.substring(0, max);
        // Check the amount of characters left for this embed
        const availableCharacters = exports.embedLimits.total - this.currentTotal;
        // If it is maxed out already return empty string as nothing can be added anymore
        if (!availableCharacters)
            return ``;
        // If the string breaks the maximum embed limit then shorten it.
        if (this.currentTotal + data.length > exports.embedLimits.total) {
            return data.substring(0, availableCharacters);
        }
        // Return the data as is with no changes.
        return data;
    }
    setAuthor(name, iconUrl, url) {
        const embed = this.getLastEmbed();
        const finalName = this.enforceLimits ? this.fitData(name, exports.embedLimits.authorName) : name;
        if (typeof iconUrl === "string") {
            embed.author = { name: finalName, iconUrl, url };
        }
        else if (iconUrl) {
            embed.author = {
                name: finalName,
                iconUrl: avatarURL(iconUrl.id, iconUrl?.discriminator, {
                    avatar: iconUrl.avatar,
                }),
                url,
            };
        }
        else {
            embed.author = { name: finalName, url };
        }
        return this;
    }
    setColor(color) {
        this.getLastEmbed().color = color.toLowerCase() === `random`
            ? // Random color
                Math.floor(Math.random() * (0xffffff + 1))
            : // Convert the hex to a acceptable color for discord
                parseInt(color.replace("#", ""), 16);
        return this;
    }
    setDescription(description) {
        if (Array.isArray(description))
            description = description.join("\n");
        this.getLastEmbed().description = this.fitData(description, exports.embedLimits.description);
        return this;
    }
    addField(name, value, inline = false) {
        const embed = this.getLastEmbed();
        if (embed.fields.length >= 25)
            return this;
        embed.fields.push({
            name: this.fitData(name, exports.embedLimits.fieldName),
            value: this.fitData(value, exports.embedLimits.fieldValue),
            inline,
        });
        return this;
    }
    addBlankField(inline = false) {
        return this.addField("\u200B", "\u200B", inline);
    }
    attachFile(file, name) {
        this.file = {
            blob: file,
            name,
        };
        this.setImage(`attachment://${name}`);
        return this;
    }
    setFooter(text, icon) {
        this.getLastEmbed().footer = {
            text: this.fitData(text, exports.embedLimits.footerText),
            iconUrl: icon,
        };
        return this;
    }
    setImage(url) {
        if (typeof url === "string")
            this.getLastEmbed().image = { url };
        else {
            this.getLastEmbed().image = {
                url: avatarURL(url.id, url.discriminator, {
                    avatar: url.avatar,
                    size: 2048,
                }),
            };
        }
        return this;
    }
    setTimestamp(time = Date.now()) {
        this.getLastEmbed().timestamp = time;
        return this;
    }
    setTitle(title, url) {
        this.getLastEmbed().title = this.fitData(title, exports.embedLimits.title);
        if (url)
            this.getLastEmbed().url = url;
        return this;
    }
    setURL(url) {
        this.getLastEmbed().url = url;
        return this;
    }
    setThumbnail(url) {
        this.getLastEmbed().thumbnail = { url };
        return this;
    }
    addEmbed(embed) {
        if (this.length === 10)
            return this;
        this.push({ ...embed, fields: embed?.fields ?? [] });
        return this;
    }
    /** Get the last DiscordEmbed, if there is no it will create one */
    getLastEmbed() {
        if (this.length)
            return this[this.length - 1];
        this.push({
            fields: [],
        });
        return this[0];
    }
    setFromJson(json) {
        if (json.author?.name)
            this.setAuthor(json.author.name, json.author.icon_url, json.url);
        if (json.title)
            this.setTitle(json.title, json.url);
        if (json.description)
            this.setDescription(json.description);
        if (json.color)
            this.setColor(json.color);
        if (json.timestamp)
            this.setTimestamp();
        if (json.footer?.text)
            this.setFooter(json.footer.text, json.footer.icon_url);
        if (json.thumbnail)
            this.setThumbnail(json.thumbnail?.url ?? json.thumbnail);
        if (json.image)
            this.setImage(json.image?.url ?? json.image);
        if (json.fields && Array.isArray(json.fields)) {
            for (const field of json.fields) {
                if (field.name && field.value) {
                    this.addField(field.name, field.value, field.inline);
                }
            }
        }
        return this;
    }
    setFromEmbed(embed) {
        if (embed.author?.name)
            this.setAuthor(embed.author.name, embed.author.iconUrl, embed.url);
        if (embed.title)
            this.setTitle(embed.title, embed.url);
        if (embed.description)
            this.setDescription(embed.description);
        if (embed.color)
            this.getLastEmbed().color = embed.color;
        if (embed.timestamp)
            this.setTimestamp(embed.timestamp);
        if (embed.footer?.text)
            this.setFooter(embed.footer.text, embed.footer.iconUrl);
        if (embed.thumbnail?.url)
            this.setThumbnail(embed.thumbnail?.url);
        if (embed.image?.url)
            this.setImage(embed.image?.url);
        if (embed.fields && Array.isArray(embed.fields)) {
            for (const field of embed.fields) {
                if (field.name && field.value) {
                    this.addField(field.name, field.value, field.inline);
                }
            }
        }
        return this;
    }
    showEmbedCode() {
        return this.map((embed) => ({
            ...embed,
            timestamp: embed.timestamp ? new Date(embed.timestamp).toISOString() : undefined,
            author: embed.author
                ? {
                    name: embed.author.name,
                    url: embed.author.url,
                    proxy_icon_url: embed.author.proxyIconUrl,
                    icon_url: embed.author.iconUrl,
                }
                : undefined,
            footer: embed.footer
                ? {
                    text: embed.footer.text,
                    proxy_icon_url: embed.footer.proxyIconUrl,
                    icon_url: embed.footer.iconUrl,
                }
                : undefined,
            image: embed.image
                ? {
                    url: embed.image.url,
                    height: embed.image.height,
                    width: embed.image.width,
                    proxy_url: embed.image.proxyUrl,
                }
                : undefined,
            thumbnail: embed.thumbnail
                ? {
                    url: embed.thumbnail.url,
                    height: embed.thumbnail.height,
                    width: embed.thumbnail.width,
                    proxy_url: embed.thumbnail.proxyUrl,
                }
                : undefined,
        }));
    }
}
exports.Embeds = Embeds;
exports.default = Embeds;

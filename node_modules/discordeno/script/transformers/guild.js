...
exports.transformGuild = void 0;
const collection_js_1 = require("../util/collection.js");
const guild_js_1 = require("./toggles/guild.js");
function transformGuild(bot, payload) {
    ...
    const guild = {
        ...
        toggles: new guild_js_1.GuildToggles(payload.guild),
        ...
        channels: new collection_js_1.Collection(payload.guild.channels?.map((channel) => {
            const result = bot.transformers.channel(bot, { channel, guildId });
            return [result.id, result];
        })),
        roles: new collection_js_1.Collection(payload.guild.roles?.map((role) => {
            const result = bot.transformers.role(bot, { role, guildId });
            return [result.id, result];
        })),
        emojis: new collection_js_1.Collection((payload.guild.emojis || []).map((emoji) => {
            const em = bot.transformers.emoji(bot, emoji);
            return [em.id, em];
        })),
        voiceStates: new collection_js_1.Collection((payload.guild.voice_states || [])

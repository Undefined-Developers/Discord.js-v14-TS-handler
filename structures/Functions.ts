...
export class ErryFunctions {
  ...
  format(millis: number, ls: LocaleString): string {
    let localization = {
  ...
  translatePermissions(permissionsArray: string[], ls: LocaleString) {
    if (!permissionsArray || permissionsArray.length <= 0) return this.client.lang.translate("common.error", ls);
    var result = permissionsArray.map((permission, index) => {
      return this.client.lang.translate(`common.permissions.${permission}`, ls)
    })
    return result;
  }
